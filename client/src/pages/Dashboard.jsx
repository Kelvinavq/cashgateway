...
const CARD_COLORS = {
  ...
  teal:    { icon: '#14b8a6', bg: 'rgba(20,184,166,0.10)'  },
  rose:    { icon: '#f43f5e', bg: 'rgba(244,63,94,0.10)'   },
  violet:  { icon: '#8b5cf6', bg: 'rgba(139,92,246,0.10)'  },
};
...
function StatCard({ label, value, Icon, colorKey = 'indigo', loading, accent }) {
  ...
  return (
    <Card sx={{ height: '100%', position: 'relative', overflow: 'hidden' }}>
      {accent && (
        <Box sx={{ position: 'absolute', top: 0, left: 0, right: 0, height: 2, bgcolor: c.icon, opacity: 0.7 }} />
      )}
      <CardContent sx={{ p: { xs: '12px !important', sm: '14px !important' } }}>
        <Stack direction="row" alignItems="center" spacing={1.5}>
          <Box sx={{
            width: 38, height: 38, borderRadius: '9px', bgcolor: c.bg,
          ...
          <Box sx={{ minWidth: 0, flex: 1 }}>
            <Typography variant="caption" color="text.secondary"
              sx={{ display: 'block', mb: 0.25, fontWeight: 500, fontSize: '0.72rem' }} noWrap>
              {label}
            </Typography>
            {loading ? (
...
export default function Dashboard() {
  ...
  const kpis = [
    ...
    { label: 'Resueltos manual',   value: s?.movements.manually_resolved,  Icon: BuildCircle,             colorKey: 'violet'  },
          ...
          <Card sx={{ height: '100%' }}>
            <Box sx={{
              ...
              <Box sx={{
                ...
                bgcolor: 'rgba(16,185,129,0.1)', borderRadius: '20px', px: 1.25, py: 0.4,
              }}>
                <Box sx={{
                ...
                <Typography sx={{ fontSize: '0.68rem', fontWeight: 600, color: '#10b981' }}>En vivo</Typography>
              </Box>
            </Box>
              ...
              <Table size="small" sx={{ minWidth: 520 }}>
                ...
                <TableBody>
                  {loading ? (
                    [...Array(6)].map((_, i) => (
                      <TableRow key={i}>
                        {[...Array(6)].map((_, j) => <TableCell key={j}><Skeleton sx={{ transform: 'none' }} /></TableCell>)}
                      </TableRow>
                    ))
                    ...
                    <TableRow key={m.id} hover
                      ...
                      <TableCell>
                        <StatusChip status={m.resolution_status} />
                      </TableCell>
                      <TableCell>
            ...
            <Box sx={{ px: { xs: 1.25, sm: 1.5 }, pb: { xs: 1.25, sm: 1.5 } }}>
              {loading ? (
                ...
                <Box sx={{ py: 4, textAlign: 'center' }}>
                  <Box sx={{
                    width: 48, height: 48, borderRadius: '50%', bgcolor: 'rgba(16,185,129,0.1)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', mx: 'auto', mb: 1.5,
                  }}>
                    <CheckCircle sx={{ color: '#10b981', fontSize: 26 }} />
                  </Box>
                  <Typography variant="body2" color="text.secondary" fontWeight={500}>Sin entregas fallidas</Typography>
                  <Typography variant="caption" color="text.secondary">Todo entregado correctamente</Typography>
                </Box>
              ) : s.recent_failed_deliveries.map((d) => (
                <Box key={d.id} sx={{
                  p: 1.5, borderRadius: '8px',
                  border: '1px solid rgba(239,68,68,0.2)',
                  bgcolor: 'rgba(239,68,68,0.06)', mb: 1,
                }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 0.5, gap: 1 }}>
                    <Typography variant="caption" fontWeight={700} color="error.main" noWrap>{d.domain_name}</Typography>
                    <Typography variant="caption" fontWeight={700} color="text.primary" noWrap>{fmtARS(d.amount)}</Typography>
                  </Box>
                  <Typography variant="caption" color="text.secondary"
